...
class MonthlyFeePaymentDetailSerializer(serializers.ModelSerializer):
    ...
    current_payment_method = serializers.CharField(
...
class MonthlyFeePaymentUpdateSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if discount_percent < 0:
            raise serializers.ValidationError(
                {'discount_percent': 'O desconto não pode ser negativo.'})
        if discount_percent > 100:
            raise serializers.ValidationError(
                {'discount_percent': 'O desconto não pode ser maior que 100%.'})

        expected_discount = (base_amount * discount_percent / Decimal('100')
                             ).quantize(Decimal('0.01'), rounding=ROUND_HALF_UP)
        if discount_value and abs(expected_discount - discount_value) > Decimal('0.05'):
            raise serializers.ValidationError(
                {'discount_value': 'Valor e porcentagem de desconto não conferem com o valor base.'})

        calculated_final = (
            base_amount - discount_value).quantize(Decimal('0.01'), rounding=ROUND_HALF_UP)
        if calculated_final < 0:
            raise serializers.ValidationError(
                {'final_amount': 'O valor final não pode ser negativo.'})

        if abs(calculated_final - final_amount) > Decimal('0.05'):
            raise serializers.ValidationError(
                {'final_amount': 'O valor final não confere com os dados informados.'})

        attrs['discount_percent'] = discount_percent.quantize(
            Decimal('0.01'), rounding=ROUND_HALF_UP)
        attrs['discount_value'] = discount_value.quantize(
            Decimal('0.01'), rounding=ROUND_HALF_UP)
        attrs['final_amount'] = final_amount.quantize(
            Decimal('0.01'), rounding=ROUND_HALF_UP)
        return attrs
    ...
    def update(self, instance, validated_data):
        ...
        instance.discount_percent = validated_data.get(
            'discount_percent', Decimal('0'))
        instance.discount_value = validated_data.get(
            'discount_value', Decimal('0'))
    ...
    def validate_quantity_installments(self, value):
        ...
        try:
            ...
            raise serializers.ValidationError(
                'Informe um número inteiro válido de parcelas.')
