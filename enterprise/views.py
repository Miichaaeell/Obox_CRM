...
class EnterpriseSettingsView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context["plans"] = Plan.objects.all().only(
            "name_plan", "price", "duration_months"
        )
...
class DownloadCashierFlowView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            return JsonResponse(
                {"status": "error", "message": "Caixa não encontrado."}, status=404
            )
...
class BillListView(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["current_created_date"] = self.request.GET.get("created_date", "")
        return context
    ...
    def get_queryset(self):
        ...
        if search:
            ...
            try:
                ...
                filters |= Q(
                    due_date__year=parsed_month.year, due_date__month=parsed_month.month
                )
            except ValueError:
                pass
...
class NFESListView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        monthlyfees = (
            MonthlyFee.objects.select_related("student", "plan")
...
class NFEAPIView(APIView):
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            return Response(
                {"status": "ok", "mensagem": "notas agendadas para emissão."},
