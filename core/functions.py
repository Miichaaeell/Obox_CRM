...
def get_context_cashier_data():
    ...
    bill = Bill.objects.select_related("payment_method", "status").filter(
        Q(date_payment__gte=(start)) | Q(date_payment__lte=(end)), cashier__isnull=True
    )
    pay = bill.filter(
    ...
    last_balance = (
        Cashier.objects.filter(status="closed").order_by("-created_at").first()
    )
    closing_balance = (
        (last_balance.closing_balance + income - expense)
        if last_balance
        else (income - expense)
    )
    expense_withdrawal = (
        last_cashier.expense_withdrawal
        if last_cashier and last_cashier.status == "open"
        else 0
    )
    context = {
        ...
        "expense_automatic": (
            pay["expense_automatic"] if pay["expense_automatic"] else 0
        ),
...
def create_new_register_cashier():
    try:
        ...
        return JsonResponse(
            {"status": "error", "title": "Erro ao abrir caixa", "message": f"{e}"},
            status=400,
        )
        ...
        return JsonResponse(
            {
                ...
                "message": f"Caixa fechado com sucesso! Saldo final R${closing_balance}, Retirado: R${withdrawalValue}",
            },
            status=200,
        )
    except Exception as e:
        return JsonResponse(
            {"status": "error", "title": "Erro ao fechar Caixa", "message": str(e)},
            status=400,
        )
...
def create_file_xlsx_cashier(cashier):
    ...
    ws["D2"], ws["E2"] = (
        "Data de fechamento",
        cashier.date_closing.strftime("%d/%m/%Y") if cashier.date_closing else "",
    )
    ...
    for b in cashier.bills.select_related("payment_method"):
        ws.cell(
            row=row,
            column=1,
            value=b.date_payment.strftime("%d/%m/%Y") if b.date_payment else "",
        )
        ws.cell(row=row, column=2, value=b.description)
        ws.cell(row=row, column=3, value="Contas a pagar")
        ws.cell(
            row=row, column=4, value=b.payment_method.method if b.payment_method else ""
        )
...
def get_context_homeview():
    today = datetime.now()
    actives_students = Student.objects.filter(status__status__iexact="Ativo")
    ...
    bill_events = (
        Bill.objects.annotate(event_date=F("due_date"))
        .values("event_date")
...
def get_dashboard_context():
    ...
    monthly_fee_values = {key: value or 0 for key, value in monthly_fee_values.items()}
    ...
    monthly_fee_counts = monthlyfees_qs.aggregate(
        ...
        due_next_count=Count(
            "id", filter=Q(paid=False, due_date__range=(today, next_week))
        ),
    )
    monthly_fee_counts = {key: value or 0 for key, value in monthly_fee_counts.items()}

    upcoming_fees_qs = monthlyfees_qs.filter(
        paid=False, due_date__range=(today, next_week)
    ).order_by("due_date")[:6]
    overdue_fees_qs = monthlyfees_qs.filter(paid=False, due_date__lt=today).order_by(
        "due_date"
    )[:6]
    ...
    def serialize_fee(fee):
        return {
            "name": (
                fee.student.name
                if fee.student
                else fee.student_name or "Sem identificação"
            ),
    ...
    payment_method_labels = [stat["method"] for stat in payment_method_stats]
    payment_method_totals = [stat["total"] for stat in payment_method_stats]
    ...
    for plan in students_by_plan:
        ...
        plan["percentage"] = (
            round((plan["count"] / total_students) * 100, 1) if total_students else 0
        )
    ...
    context = {
        ...
        "monthly_fee_distribution": {
            "paid_pct": (
                round(
                    (monthly_fee_counts["paid_count"] / monthly_fee_counts["total"])
                    * 100,
                    1,
                )
                if monthly_fee_counts["total"]
                else 0
            ),
            "pending_pct": (
                round(
                    (monthly_fee_counts["pending_count"] / monthly_fee_counts["total"])
                    * 100,
                    1,
                )
                if monthly_fee_counts["total"]
            ...
            "overdue_pct": (
                round(
                    (monthly_fee_counts["overdue_count"] / monthly_fee_counts["total"])
                    * 100,
                    1,
                )
                if monthly_fee_counts["total"]
        ...
        "payment_method_labels": mark_safe(json.dumps(payment_method_labels)),
        "payment_method_totals": mark_safe(json.dumps(payment_method_totals)),
        "monthly_cashflow_labels": mark_safe(json.dumps(monthly_cashflow_labels)),
        "monthly_cashflow_billed": mark_safe(json.dumps(monthly_cashflow_billed)),
        "monthly_cashflow_received": mark_safe(json.dumps(monthly_cashflow_received)),
